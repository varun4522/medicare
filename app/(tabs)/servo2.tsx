...
type Servo2ControlRow = {
  id: number;
  status: number | null;
};

const SERVO2_ROW_ID = 1;

export default function Servo2Screen() {
  const [status, setStatus] = useState<number>(0);
  const [loading, setLoading] = useState(true);
  const [updating, setUpdating] = useState(false);

  const fetchServo2Control = useCallback(async () => {
    setLoading(true);
    ...
    const { data, error } = await supabase
      ...
      .select('id, status')
      .eq('id', SERVO2_ROW_ID)
      .maybeSingle<Servo2ControlRow>();

    if (error) {
      setLoading(false);
      Alert.alert('Servo2 sync failed', error.message);
      return;
    }
    ...
    if (!data) {
      const { data: insertedRow, error: insertError } = await supabase
        .from('servo2_control')
        .upsert({ id: SERVO2_ROW_ID, status: 0 }, { onConflict: 'id' })
        .select('id, status')
        .single<Servo2ControlRow>();

      setLoading(false);

      if (insertError) {
        Alert.alert('Servo2 setup failed', insertError.message);
        return;
      }
  ...
  const handleToggle = useCallback(
    async (nextValue: boolean) => {
      ...
      const { data, error } = await supabase
        .from('servo2_control')
        .upsert({ id: SERVO2_ROW_ID, status: nextStatus }, { onConflict: 'id' })
        .select('id, status')
        .single<Servo2ControlRow>();
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.container}>
        <Text style={styles.pageTitle}>Servo2 Control</Text>
        <Text style={styles.pageSub}>
          Connected to Supabase table `servo2_control` using row id {SERVO2_ROW_ID}.
        </Text>
        ...
        <View style={styles.card}>
          ...
          {loading ? (
            <View style={styles.loaderWrap}>
              ...
              <Text style={styles.loaderText}>Loading servo2 status...</Text>
            </View>
          ) : (
            <>
                ...
                {updating ? (
                  ...
                  <>
                    <Ionicons
                      ...
                      color={Colors.white}
                    />
                    <Text style={styles.buttonText}>{isOn ? 'Turn Servo2 Off' : 'Turn Servo2 On'}</Text>
                  </>
                )}
              </Pressable>

              <Pressable style={styles.secondaryButton} onPress={() => void fetchServo2Control()}>
